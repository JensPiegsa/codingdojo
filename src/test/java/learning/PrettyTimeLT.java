...
public class PrettyTimeLT {
    ...
    void useOfPrettyTime() {
        ...
        final String format = prettyTime.format(LocalDateTime.now().minusDays(1));
    ...
    void compoundDuration() {
        final PrettyTime prettyTime = new PrettyTime(Locale.ENGLISH);
        final String format = prettyTime.format(LocalDateTime.now().minusDays(1).minusHours(5));
        then(format).isEqualTo("1 day ago");
    }
    ...
    void canFormat59MinDuration() {
        final PrettyTime prettyTime = new PrettyTime(Locale.ENGLISH);
        final String format = prettyTime.format(LocalDateTime.now().minusMinutes(59));
    ...
    void canFormat60MinDuration() {
        final PrettyTime prettyTime = new PrettyTime(Locale.ENGLISH);
        final String format = prettyTime.format(LocalDateTime.now().minusMinutes(60));
    ...
    void canCalcDurations() {
        ...
        final LocalDateTime creationTime = now.minusMinutes(10).minusHours(7);
        
        final List<Duration> durations = prettyTime.calculatePreciseDuration(creationTime);
        final String format = prettyTime.formatDuration(durations);
    ...
    @Test @DisplayName("can format 60 min duration with reference")
    void canFormat60MinDurationWithReference() {
        final LocalDateTime now = LocalDateTime.now();
        final PrettyTime prettyTime = new PrettyTime(Locale.ENGLISH);
        prettyTime.setReference(now);
        final String format = prettyTime.format(LocalDateTime.now().minusMinutes(60));
